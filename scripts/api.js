...
function handleRequestWithRetry(requestFn, options, callbackData, callbacks) {
    try {
        ...
        sys.logs.info("[gmail] Handling request "+JSON.stringify(error));
        dependencies.oauth.functions.refreshToken('gmail:refreshToken');
...
exports.getAccessToken = function () {
    sys.logs.info("[gmail] Getting access token from oauth");
    return dependencies.oauth.functions.connectUser('gmail:userConnected');
}
...
exports.removeAccessToken = function () {
    sys.logs.info("[gmail] Removing access token from oauth");
    return dependencies.oauth.functions.disconnectUser('gmail:disconnectUser');
}
...
exports.get = function(path, httpOptions, callbackData, callbacks) {
    ...
    return httpService.get(Gmail(options), callbackData, callbacks);
};
...
exports.post = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.post(Gmail(options), callbackData, callbacks);
};
...
exports.put = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.put(Gmail(options), callbackData, callbacks);
};
...
exports.patch = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.patch(Gmail(options), callbackData, callbacks);
};
...
exports.delete = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.delete(Gmail(options), callbackData, callbacks);
};
...
exports.head = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.head(Gmail(options), callbackData, callbacks);
};
...
exports.options = function(path, httpOptions, callbackData, callbacks) {
    let options = checkHttpOptions(path, httpOptions);
    return httpService.options(Gmail(options), callbackData, callbacks);
};
...
exports.utils = {
    ...
    getConfiguration: function (property) {
        if (!property) {
            sys.logs.debug('[Gmail] Get configuration');
            return JSON.stringify(config.get());
        }
        sys.logs.debug('[Gmail] Get property: '+property);
...
function setApiUri(options) {
    ...
    options.url = config.get("GMAIL_API_BASE_URL") + url;
    sys.logs.debug('[gmail] Set url: ' + options.path + "->" + options.url);
    return options;
}
...
function setAuthorization(options) {
    ...
    sys.logs.debug('[gmail] setting authorization');
    let pkgConfig = config.get();
    sys.logs.debug('[gmail] config: '+JSON.stringify(pkgConfig));
    sys.logs.debug('[gmail] config id: '+JSON.stringify(pkgConfig.id));
